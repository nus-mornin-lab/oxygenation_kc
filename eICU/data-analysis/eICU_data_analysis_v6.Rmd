...
save(patient, O2measurement, file = "RData/eICU_data_analysis_v6_checkpoint1.RData")
```
...
load("RData/eICU_data_analysis_v6_checkpoint1.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v6_checkpoint_vent.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v6_checkpoint_vent2.RData")
```

```{r include=FALSE, eval=TRUE}
load(file = "RData/eICU_data_analysis_v6_checkpoint_vent2.RData")
```
...
tmp <- tmp[!(tmp %in% patient$icustay_id[patient$first_stay & patient$vent_end - patient$vent_start >= time_limit])]
...
patient$nOxy[which(is.na(patient$nOxy) & patient$vent_end - patient$vent_start >= time_limit)] <- 0


# We've counted all oxygen measurements.
# We should only be counting the measurements that happen during ventilation
tmp_IDs <- patient$icustay_id[which(patient$vent_end - patient$vent_start >= time_limit & patient$nOxy >= nOxy_limit)]
n <- length(tmp_IDs)
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v6_checkpoint_vent2_2.RData")
```

```{r include=FALSE, eval=TRUE}
load(file = "RData/eICU_data_analysis_v6_checkpoint_vent2_2.RData")
```

```{r}
mean(patient$nOxy[which(patient$vent_end - patient$vent_start >= time_limit)] >= nOxy_limit)
```
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v6_checkpoint_vent2_3.RData")
```

```{r include=FALSE, eval=TRUE}
load(file = "RData/eICU_data_analysis_v6_checkpoint_vent2_3.RData")
```
...
tmp <- which(patient$vent_end - patient$vent_start >= time_limit)
...
### We do note subset by `high_vent_proportion` since this is not recorded for those on supplementary oxygen.
...
# # Delete those with no `high_vent_proportion`
# tmp <- is.na(patient_subset$high_vent_proportion)
# cat("\nPatients with no tidal volume records during ventilation:", sum(tmp), "out of", nrow(patient_subset))
# delete <- tmp
# patient_subset <- patient_subset[!delete,]
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v6_checkpoint_vent3.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image("RData/eICU_data_analysis_v6_checkpoint2.RData")
```
...
load("RData/eICU_data_analysis_v6_checkpoint2.RData")
```
...
if(!isTRUE(all.equal(att, round(att, digits = 2)))) {
  axis(2, at = att, lab = paste0(sprintf('%.1f', att*100), '%'), las = TRUE)
    ...
    if("high_vent_proportion" %in% colnames(gamfitMed$model)) {
      eval(parse(text = paste(c('predictionsPlusCI <- predict(gamfitMed, newdata = data.frame(',
                              xName, ' = gamfitMed$model$', xName, ", gender = 'F', age = median(gamfitMed$model$age), hospital_id = 264,
            high_vent_proportion = median(gamfitMed$model$high_vent_proportion)
          ), type = 'link', se.fit = T)"), collapse = "")))  
    } else {
    eval(parse(text = paste(c('predictionsPlusCI <- predict(gamfitMed, newdata = data.frame(',
...
gamfitMed_type <- gam(mortality_in_ICU ~ s(median)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type == 1,], family = binomial)
summary(gamfitMed_type)
gam_plot(gamfitMed_type, main = "Supplementary oxygen")

gamfitMed_type <- gam(mortality_in_ICU ~ s(median)+gender+s(age) + s(high_vent_proportion), data = patient_subset[patient_subset$oxygen_therapy_type >= 2,], family = binomial)
summary(gamfitMed_type)
gam_plot(gamfitMed_type, main = "Mechanical ventilation")

gamfitMed_type <- gam(mortality_in_ICU ~ s(median)+gender+s(age) + s(high_vent_proportion), data = patient_subset[patient_subset$oxygen_therapy_type == 3,], family = binomial)
summary(gamfitMed_type)
gam_plot(gamfitMed_type, main = "Noninvasive ventilation")

gamfitMed_type <- gam(mortality_in_ICU ~ s(median)+gender+s(age) + s(high_vent_proportion), data = patient_subset[patient_subset$oxygen_therapy_type == 4,], family = binomial)
summary(gamfitMed_type)
gam_plot(gamfitMed_type, main = "Invasive ventilation")
```
...
```{r gamfitProp}
gam_plot <- function(gamfitProp, main = "Effect of treatment regime"){
...
if(!isTRUE(all.equal(att, round(att, digits = 2)))) {
  axis(2, at = att, lab = paste0(sprintf('%.1f', att*100), '%'), las = TRUE)
    ...
    if("high_vent_proportion" %in% colnames(gamfitProp$model)) {
      eval(parse(text = paste(c('predictionsPlusCI <- predict(gamfitProp, newdata = data.frame(',
                              xName, ' = gamfitProp$model$', xName, ", gender = 'F', age = median(gamfitProp$model$age),
...
gamfitProp_type <- gam(mortality_in_ICU ~ s(prop)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type == 1,], family = binomial)
summary(gamfitProp_type)
gam_plot(gamfitProp_type, main = "Supplementary oxygen")

gamfitProp_type <- gam(mortality_in_ICU ~ s(prop)+gender+s(age) + s(high_vent_proportion), data = patient_subset[patient_subset$oxygen_therapy_type >= 2,], family = binomial)
summary(gamfitProp_type)
gam_plot(gamfitProp_type, main = "Mechanical ventilation")

gamfitProp_type <- gam(mortality_in_ICU ~ s(prop)+gender+s(age) + s(high_vent_proportion), data = patient_subset[patient_subset$oxygen_therapy_type == 3,], family = binomial)
summary(gamfitProp_type)
gam_plot(gamfitProp_type, main = "Noninvasive ventilation")

gamfitProp_type <- gam(mortality_in_ICU ~ s(prop)+gender+s(age) + s(high_vent_proportion), data = patient_subset[patient_subset$oxygen_therapy_type == 4,], family = binomial)
summary(gamfitProp_type)
gam_plot(gamfitProp_type, main = "Invasive ventilation")
```
