...
save(patient, O2measurement, file = "RData/eICU_data_analysis_v5_checkpoint1.RData")
```
...
load("RData/eICU_data_analysis_v5_checkpoint1.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v5_checkpoint_vent.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v5_checkpoint_vent2.RData")
```

```{r include=FALSE, eval=TRUE}
load(file = "RData/eICU_data_analysis_v5_checkpoint_vent2.RData")
```
...
We only count measurements that happen during the first oxygen therapy session, and we are only interested in those receiving oxygen for at least 48 hours.
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v5_checkpoint_vent2_2.RData")
```

```{r include=FALSE, eval=TRUE}
load(file = "RData/eICU_data_analysis_v5_checkpoint_vent2_2.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/eICU_data_analysis_v5_checkpoint_vent3.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image("RData/eICU_data_analysis_v5_checkpoint2.RData")
```
...
load("RData/eICU_data_analysis_v5_checkpoint2.RData")
```
...
gamfitMed_type <- gam(mortality_in_ICU ~ s(median)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type >= 2,], family = binomial)
summary(gamfitMed_type)
gam_plot(gamfitMed_type, main = "Mechanical ventilation")

gamfitMed_type <- gam(mortality_in_ICU ~ s(median)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type == 3,], family = binomial)
summary(gamfitMed_type)
gam_plot(gamfitMed_type, main = "Noninvasive ventilation")

gamfitMed_type <- gam(mortality_in_ICU ~ s(median)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type == 4,], family = binomial)
summary(gamfitMed_type)
...
gamfitProp_type <- gam(mortality_in_ICU ~ s(prop)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type >= 2,], family = binomial)
summary(gamfitProp_type)
gam_plot(gamfitProp_type, main = "Mechanical ventilation")

gamfitProp_type <- gam(mortality_in_ICU ~ s(prop)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type == 3,], family = binomial)
summary(gamfitProp_type)
gam_plot(gamfitProp_type, main = "Noninvasive ventilation")

gamfitProp_type <- gam(mortality_in_ICU ~ s(prop)+gender+s(age), data = patient_subset[patient_subset$oxygen_therapy_type == 4,], family = binomial)
summary(gamfitProp_type)
