...
Count the number of oxygen measurements per ICU stay
...
tmp <- tmp[!(tmp %in% patient$icustay_id[patient$first_stay])]
...
patient$nOxy[which(is.na(patient$nOxy) & patient$first_stay)] <- 0

mean(patient$nOxy[which(patient$first_stay)] >= 72)
```
...
for(i in 1:n) {
  tmp <- O2measurement$spO2_Value[O2measurement$icustay_id == patient_subset$icustay_id[i]]
