...
save.image(file = "RData/data_analysis_v2_checkpoint1.RData")
```
...
load("RData/data_analysis_v2_checkpoint1.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/data_analysis_v2_checkpoint_vent2_3.RData")
```
...
# Only keep those with fiO2 above 21% at least once during their oxygen therapy session.
keep <- patient_subset$max_fiO2 > 21
cat("\nPatients with no fiO2 recorded:", sum(is.na(keep)))
cat("\nPatients with no fiO2 above 21%:", sum(keep == FALSE, na.rm = TRUE))
...
Hospital will be used as a random effect. Having hospitals only appear once in the data makes model estimation computationally more challenging.
...
delete <- pat_eICU_subset$hospital_id %in% as.numeric(names(tmp[tmp < 2]))
...
```{r include=FALSE, eval=FALSE}
save.image(file = "RData/data_analysis_v2_checkpoint_vent3.RData")
```
...
```{r include=FALSE, eval=FALSE}
save.image("RData/data_analysis_v2_checkpoint2.RData")
```
...
```{r}
gamfitMed_eICU <- gamm(mortality_in_Hospt ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset, family = binomial, random = list(hospital_id = ~ 1), contol=list(sing.tol=.1, maxIter = 500, msMaxIter = 500, msMaxEval = 500, tolerance = 0.1, msTol = 0.1))$gam

gamfitMed_MIMIC <- gam(mortality_in_ICU ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset, family = binomial)
```


```{r gamfitMed}
pdf(file = "Figures/med_v2.pdf", width = plot_width,
...
```{r}
gamfitProp_eICU <- gamm(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset, family = binomial, random = list(hospital_id = ~ 1))$gam

gamfitProp_below_eICU <- gamm(mortality_in_Hospt ~ s(propBelow)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset, family = binomial, random = list(hospital_id = ~ 1))$gam

gamfitProp_above_eICU <- gamm(mortality_in_Hospt ~ s(propAbove)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset, family = binomial, random = list(hospital_id = ~ 1))$gam


gamfitProp_MIMIC <- gam(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset, family = binomial)

gamfitProp_below_MIMIC <- gam(mortality_in_Hospt ~ s(propBelow)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset, family = binomial)

gamfitProp_above_MIMIC <- gam(mortality_in_Hospt ~ s(propAbove)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset, family = binomial)
```

```{r gamfitProp}
pdf(file = "Figures/prop_v2.pdf", width = plot_width,
...
```{r include=FALSE, eval=FALSE}
save.image("RData/data_analysis_v2_checkpoint3.RData")
```
...
We also compute the estimted mortality after categorizing `prop` into 3 groups.

```{r}
pat_eICU_subset$prop_cat <- as.factor((pat_eICU_subset$prop >= .4) + (pat_eICU_subset$prop >= .8))
levels(pat_eICU_subset$prop_cat) <- c("<.4", "[.4, .8)", ">= .8")


tab3 <- gamm(mortality_in_Hospt ~ prop_cat+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset, family = binomial, random = list(hospital_id = ~ 1))$gam
tmp <- predict(tab3, newdata = data.frame(
  prop_cat = levels(pat_eICU_subset$prop_cat),
  gender = 'F',
  age = median(tab3$model$age),
  bmi = median(tab3$model$bmi),
  sofatotal = median(tab3$model$sofatotal)
), type = 'link', se.fit= TRUE)
logistic(tmp$fit - 2*tmp$se.fit)
logistic(tmp$fit)
logistic(tmp$fit + 2*tmp$se.fit)
...
pat_MIMIC_subset$prop_cat <- as.factor((pat_MIMIC_subset$prop >= .4) + (pat_MIMIC_subset$prop >= .8))
levels(pat_MIMIC_subset$prop_cat) <- c("<.4", "[.4, .8)", ">= .8")


tab3 <- gam(mortality_in_Hospt ~ prop_cat+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset, family = binomial)
tmp <- predict(tab3, newdata = data.frame(
  prop_cat = levels(pat_MIMIC_subset$prop_cat),
  gender = 'F',
  age = median(tab3$model$age),
  bmi = median(tab3$model$bmi),
  sofatotal = median(tab3$model$sofatotal)
...
pat_MIMIC_subset$propAbove_cat <- as.factor((pat_MIMIC_subset$propAbove >= .4) + (pat_MIMIC_subset$propAbove >= .8))
levels(pat_MIMIC_subset$propAbove_cat) <- c("<.4", "[.4, .8)", ">= .8")

tab3 <- gam(mortality_in_Hospt ~ propAbove_cat+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset, family = binomial)
tmp <- predict(tab3, newdata = data.frame(
  propAbove_cat = levels(pat_MIMIC_subset$propAbove_cat),
  gender = 'F',
  age = median(tab3$model$age),
  bmi = median(tab3$model$bmi),
  sofatotal = median(tab3$model$sofatotal)
...
```{r apache_model_fit}
gamfitMed_eICU_tmp <- gamm(mortality_in_Hospt ~ s(median)+gender+s(age)+s(bmi)+s(apsiii), data = pat_eICU_subset, family = binomial, random = list(hospital_id = ~ 1), contol=list(sing.tol=.1, maxIter = 500, msMaxIter = 500, msMaxEval = 500, tolerance = 0.1, msTol = 0.1))$gam

gamfitMed_MIMIC_tmp <- gam(mortality_in_ICU ~ s(median)+gender+s(age)+s(bmi)+s(apsiii), data = pat_MIMIC_subset, family = binomial)


gamfitProp_eICU_tmp <- gamm(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(apsiii), data = pat_eICU_subset, family = binomial, random = list(hospital_id = ~ 1))$gam

gamfitProp_MIMIC_tmp <- gam(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(apsiii), data = pat_MIMIC_subset, family = binomial)
```

```{r apache}
pdf(file = "Figures/med_APACHE_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
...
pdf(file = "Figures/prop_APACHE_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
par(mfcol = c(1,2), cex = 1)
...
```{r model_fit_24}
gamfitMed_eICU_tmp <- gamm(mortality_in_Hospt ~ s(median_24)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$nOxy_24 >= 12,], family = binomial, random = list(hospital_id = ~ 1), contol=list(sing.tol=.1, maxIter = 500, msMaxIter = 500, msMaxEval = 500, tolerance = 0.1, msTol = 0.1))$gam

gamfitMed_MIMIC_tmp <- gam(mortality_in_ICU ~ s(median_24)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$nOxy_24 >= 12,], family = binomial)


gamfitProp_eICU_tmp <- gamm(mortality_in_Hospt ~ s(prop_24)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$nOxy_24 >= 12,], family = binomial, random = list(hospital_id = ~ 1))$gam

gamfitProp_MIMIC_tmp <- gam(mortality_in_Hospt ~ s(prop_24)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$nOxy_24 >= 12,], family = binomial)
```

```{r limit_24}
pdf(file = "Figures/med_24_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
...
pdf(file = "Figures/prop_24_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
par(mfcol = c(1,2), cex = 1)
...
```{r model_fit_48}
gamfitMed_eICU_tmp <- gamm(mortality_in_Hospt ~ s(median_48)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$nOxy_48 >= 24,], family = binomial, random = list(hospital_id = ~ 1), contol=list(sing.tol=.1, maxIter = 500, msMaxIter = 500, msMaxEval = 500, tolerance = 0.1, msTol = 0.1))$gam

gamfitMed_MIMIC_tmp <- gam(mortality_in_ICU ~ s(median_48)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$nOxy_48 >= 24,], family = binomial)


gamfitProp_eICU_tmp <- gamm(mortality_in_Hospt ~ s(prop_48)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$nOxy_48 >= 24,], family = binomial, random = list(hospital_id = ~ 1))$gam

gamfitProp_MIMIC_tmp <- gam(mortality_in_Hospt ~ s(prop_48)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$nOxy_48 >= 24,], family = binomial)
```

```{r limit_48}
pdf(file = "Figures/med_48_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
...
pdf(file = "Figures/prop_48_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
par(mfcol = c(1,2), cex = 1)
...
```{r model_fit_72}
gamfitMed_eICU_tmp <- gamm(mortality_in_Hospt ~ s(median_72)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$nOxy_72 >= 36,], family = binomial, random = list(hospital_id = ~ 1), contol=list(sing.tol=.1, maxIter = 500, msMaxIter = 500, msMaxEval = 500, tolerance = 0.1, msTol = 0.1))$gam

gamfitMed_MIMIC_tmp <- gam(mortality_in_ICU ~ s(median_72)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$nOxy_72 >= 36,], family = binomial)


gamfitProp_eICU_tmp <- gamm(mortality_in_Hospt ~ s(prop_72)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$nOxy_72 >= 36,], family = binomial, random = list(hospital_id = ~ 1))$gam

gamfitProp_MIMIC_tmp <- gam(mortality_in_Hospt ~ s(prop_72)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$nOxy_72 >= 36,], family = binomial)
```

```{r limit_72}
pdf(file = "Figures/med_72_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
...
pdf(file = "Figures/prop_72_v2.pdf", width = plot_width,
    height = 3, pointsize = point_size, family = font)
par(mfcol = c(1,2), cex = 1)
...
gamfitMed_eICU_tmp_1 <- gamm(mortality_in_Hospt ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$oxygen_therapy_type == 4,], family = binomial, random = list(hospital_id = ~ 1), contol=list(sing.tol=.1, maxIter = 500, msMaxIter = 500, msMaxEval = 500, tolerance = 0.1, msTol = 0.1))$gam
gamfitMed_eICU_tmp_2 <- gamm(mortality_in_Hospt ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$oxygen_therapy_type %in% c(1,3),], family = binomial, random = list(hospital_id = ~ 1), contol=list(sing.tol=.1, maxIter = 500, msMaxIter = 500, msMaxEval = 500, tolerance = 0.1, msTol = 0.1))$gam

gamfitMed_MIMIC_tmp_1 <- gam(mortality_in_ICU ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$oxygen_therapy_type == 4,], family = binomial)
gamfitMed_MIMIC_tmp_2 <- gam(mortality_in_ICU ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$oxygen_therapy_type %in% c(1,3),], family = binomial)


gamfitProp_eICU_tmp_1 <- gamm(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$oxygen_therapy_type == 4,], family = binomial, random = list(hospital_id = ~ 1))$gam
gamfitProp_eICU_tmp_2 <- gamm(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$oxygen_therapy_type %in% c(1,3),], family = binomial, random = list(hospital_id = ~ 1))$gam

gamfitProp_MIMIC_tmp_1 <- gam(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$oxygen_therapy_type == 4,], family = binomial)
gamfitProp_MIMIC_tmp_2 <- gam(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$oxygen_therapy_type %in% c(1,3),], family = binomial)
```

```{r vent_type}
pdf(file = "Figures/med_vent_type_v2.pdf", width = plot_width,
    height = 6, pointsize = point_size, family = font)
...
pdf(file = "Figures/prop_vent_type_v2.pdf", width = plot_width,
    height = 6, pointsize = point_size, family = font)
par(mfcol = c(2,2), cex = 1)
...
pdf(file = "Figures/med_ICU_type_v2.pdf", width = plot_width,
    height = 12, pointsize = point_size, family = font)

par(mfrow = c(5,2), cex = 1)

for(i in 1:5) {
gam_plotMed(gam(mortality_in_Hospt ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$unit_type == mat[i,1],], family = binomial),
            main = mat[i,3], label = c("A", "C", "E", "G", "I")[i])

gam_plotMed(gam(mortality_in_ICU ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$unit_type == mat[i,2],], family = binomial),
...
pdf(file = "Figures/prop_ICU_type_v2.pdf", width = plot_width,
    height = 12, pointsize = point_size, family = font)

par(mfrow = c(5,2), cex = 1)

for(i in 1:5) {
gam_plotProp(gam(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$unit_type == mat[i,1],], family = binomial),
            main = mat[i,3], label = c("A", "C", "E", "G", "I")[i])

gam_plotProp(gam(mortality_in_ICU ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$unit_type == mat[i,2],], family = binomial),
            main = mat[i,4], label = c("B", "D", "F", "H", "J")[i])
...
pdf(file = "Figures/med_ethnicity_v2.pdf", width = plot_width,
    height = 12, pointsize = point_size, family = font)

par(mfrow = c(n_cat,2), cex = 1)

for(i in 1:n_cat) {
gam_plotMed(gam(mortality_in_Hospt ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$eth == mat[i,1],], family = binomial),
            main = paste0(ifelse(i == 1, "eICU-CRD\n", ""), mat[i,2]), label = c("A", "C", "E", "G", "I")[i])

gam_plotMed(gam(mortality_in_ICU ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$eth == mat[i,1],], family = binomial),
...
pdf(file = "Figures/prop_ethnicity_v2.pdf", width = plot_width,
    height = 12, pointsize = point_size, family = font)

par(mfrow = c(n_cat,2), cex = 1)

for(i in 1:n_cat) {
gam_plotProp(gam(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[pat_eICU_subset$eth == mat[i,1],], family = binomial),
            main = paste0(ifelse(i == 1, "eICU-CRD\n", ""), mat[i,2]), label = c("A", "C", "E", "G", "I")[i])

gam_plotProp(gam(mortality_in_ICU ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[pat_MIMIC_subset$eth == mat[i,1],], family = binomial),
            main = paste0(ifelse(i == 1, "MIMIC\n", ""), mat[i,2]), label = c("B", "D", "F", "H", "J")[i])
...
pdf(file = paste0("Figures/", mat[i,2], "_v2.pdf"), width = plot_width,
    height = 6, pointsize = point_size, family = font)

par(mfrow = c(2,2), cex = 1)

gam_plotMed(gam(mortality_in_Hospt ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[which(pat_eICU_subset[,mat[i,1]]),], family = binomial),
            main = "eICU-CRD", label = "A", yRange = 0:1)

gam_plotMed(gam(mortality_in_ICU ~ s(median)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[which(pat_MIMIC_subset[,mat[i,1]]),], family = binomial),
            main = "MIMIC", label = "B", yRange = 0:1)


gam_plotProp(gam(mortality_in_Hospt ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_eICU_subset[which(pat_eICU_subset[,mat[i,1]]),], family = binomial),
            main = "", label = "C", yRange = 0:1)

gam_plotProp(gam(mortality_in_ICU ~ s(prop)+gender+s(age)+s(bmi)+s(sofatotal), data = pat_MIMIC_subset[which(pat_MIMIC_subset[,mat[i,1]]),], family = binomial),
...
```{r}
pdf(file = "Figures/hist_v2.pdf", width = plot_width,
    height = 6, pointsize = point_size, family = font)
